...
describe("ProgramImplementation", function () {
  ...
  describe("constructor", () => {
    it("ProgramImplementation SHOULD deploy properly", async () => {
      ...
      programImplementationArtifact = await artifacts.readArtifact(
        "ProgramImplementation"
      );
      programImplementation = <ProgramImplementation>(
        await deployContract(user, programImplementationArtifact, [])
      );

      // Verify deploy
      expect(
        isAddress(programImplementation.address),
        "Failed to deploy ProgramImplementation"
      ).to.be.true;
    });
  });
  ...
  describe("core functions", () => {
    ...
    describe("test: initialize", async () => {
      ...
      it("default values MUST match the arguments while invoking initialize", async () => {
        // check roles
        expect(await programImplementation.PROGRAM_OPERATOR_ROLE()).equals(
          PROGRAM_OPERATOR_ROLE
        );
        expect(await programImplementation.DEFAULT_ADMIN_ROLE()).equals(
          DEFAULT_ADMIN_ROLE
        );
        ...
        expect(
          await programImplementation.getRoleMemberCount(DEFAULT_ADMIN_ROLE)
        ).equals(_adminRoles.length);
        expect(
          await programImplementation.getRoleMember(DEFAULT_ADMIN_ROLE, 0)
        ).equals(_adminRoles[0]);

        expect(
          await programImplementation.getRoleMemberCount(PROGRAM_OPERATOR_ROLE)
        ).equals(_programOperators.length);
        expect(
          await programImplementation.getRoleMember(PROGRAM_OPERATOR_ROLE, 0)
        ).equals(_programOperators[0]);
        expect(
          await programImplementation.getRoleMember(PROGRAM_OPERATOR_ROLE, 1)
        ).equals(_programOperators[1]);
      });
      ...
      it("initialize CANNOT not be invoked on already initialized contract ", async () => {
        ...
        await expect(
          programImplementation.initialize(encodeProgramParameters(params))
    ...
    describe("test: updateMetaPtr", async () => {
      ...
      it("updateMetaPtr SHOULD revert if invoked by wallet who is not program operator", async () => {
        ...
        const newProgramImplementation = <ProgramImplementation>(
          await deployContract(user, programImplementationArtifact, [])
        );
        ...
        await expect(
          newProgramImplementation.updateMetaPtr(randomMetaPtr)
        ).to.revertedWith(
