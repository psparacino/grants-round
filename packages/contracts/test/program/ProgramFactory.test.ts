...
describe("ProgramFactory", function () {
  ...
  describe("constructor", () => {
    it("ProgramFactory SHOULD deploy properly", async () => {
      ...
      programContractFactory = await ethers.getContractFactory(
        "ProgramFactory"
      );
      programFactory = <ProgramFactory>(
        await upgrades.deployProxy(programContractFactory)
      );

      // Verify deploy
      expect(
        isAddress(programFactory.address),
        "Failed to deploy ProgramFactory"
      ).to.be.true;
    });
  });
  ...
  describe("core functions", () => {
    ...
    describe("test: updateProgramContract", async () => {
      it("ProgramContract SHOULD have default address after deploy ", async () => {
        expect(await programFactory.programContract()).to.be.equal(AddressZero);
      });

      it("ProgramContract SHOULD emit ProgramContractUpdated event after invoking updateProgramContract", async () => {
        await expect(
          programFactory.updateProgramContract(programImplementation.address)
        )
      ...
      it("ProgramContract SHOULD have program address after invoking updateProgramContract", async () => {
        await programFactory
          .updateProgramContract(programImplementation.address)
          .then(async () => {
